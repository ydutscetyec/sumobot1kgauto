 ...
 *   Portal .......... two columns from here down:
 *                       main : Lente, Opinyon + Lathalain, Isports
 *                       side : promo, Napapanahon-Laganap tabs, Mga Bagong Posts
 *   Nakaligtaan ..... five-across catch-up row
 *
 * The page runs full width through Balita so the lead stays impactful, then
 * narrows into the portal column. Every section degrades to an empty-state
 * card so a thin site still looks finished.
 *
 * Queries run up front rather than inline so each one can exclude everything
 * already placed above it — without that, the same post appears in four
 * bands on a site with only a handful of articles.
 */
get_header();
 ...
 *
 * The Multimedia card shows the video itself rather than a thumbnail that
 * links away — a video section whose videos do not play is a strange thing.
 * Returns '' when the post has no embed, so the caller can fall back to the
 * featured image.
 */
...
/* Anything the page has not shown yet. Falls back to the newest posts on a
   small site, so the row is never an empty box. */
...
<main class="home">
  ...
  <!-- ==================================================================
       Portal: everything below runs in two columns. The sidebar is a
       single sticky rail rather than one panel per band, so it stays with
       the reader through Lente, the columns and Isports.
       ================================================================== -->
      ...
      <section class="band band--first" id="multimedia">
        ...
        <?php if ( $lente->have_posts() ) : while ( $lente->have_posts() ) : $lente->the_post(); ?>
          ...
          <article class="lente-card">
            <?php if ( $embed ) : ?>
              <?php
              /* The video plays here rather than sending the reader to the
                 single-post page. Vertical clips (YouTube Shorts, Reels) get
                 a portrait frame — at 16/9 they would sit in a wide black
                 box with the content squeezed into the middle third. */
              $is_short = false !== strpos( get_post_field( 'post_content', get_the_ID() ), '/shorts/' );
              ?>
              <div class="lente-card__embed<?php echo $is_short ? ' lente-card__embed--portrait' : ''; ?>">
                <?php echo $embed; // Already escaped by the_content filters. ?>
              </div>
            <?php else : ?>
        ...
        <div class="tabs" id="sideTabs">
          ...
          <div class="tabs__panel" id="tab-laganap" role="tabpanel" aria-labelledby="btn-laganap" hidden>
            ...
            <?php endif; ?>
                 ...
                 <!-- Hanapin ang Salita Word Search -->
      ...
      <?php if ( $nakaligtaan->have_posts() ) : $i = 0; while ( $nakaligtaan->have_posts() ) : $nakaligtaan->the_post(); ?>
        <article class="card">
          <a class="card__media" href="<?php the_permalink(); ?>" tabindex="-1" aria-hidden="true">
            ...
            <span class="tag tag--solid tag--pinned"><?php echo esc_html( angbantayog_badge_label( get_the_ID() ) ); ?></span>
          </a>
          <h3 class="card__title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
          <p class="byline"><?php echo esc_html( get_the_author() ); ?> &middot; <?php echo esc_html( get_the_date() ); ?></p>
        </article>
