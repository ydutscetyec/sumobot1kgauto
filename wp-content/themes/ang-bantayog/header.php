...
<header class="site-header">
  ...
  <nav class="navbar" id="navbar" aria-label="<?php esc_attr_e( 'Pangunahing menu', 'ang-bantayog' ); ?>">
    <div class="container nav__inner">
      ...
      <?php
      /**
       * The search field sits inside nav__inner so it opens beside the icon
       * rather than as a bar under the menu. It has no .container wrapper —
       * nav__inner is already the container, and nesting a second one is
       * what forced the field to full page width.
       */
      ?>
   ...
   * Section banner.
   *
   * Sits below the nav so every page reads masthead -> menu -> section.
   * The image itself is set in style.css from the body class, which means
   * adding a section later is a CSS line, not a template edit.
   */
  if ( is_category() || ( is_singular( 'post' ) && has_category() ) ) :
    ?>
